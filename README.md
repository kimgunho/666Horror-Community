...
firebase를 이용하여 프로젝트의 로그인과 게시물의 DB를 해결할 예정입니다.
...
**컨셉** : <br />
회원가입은 기본적인 이메일과, 비밀번호, 닉네임만을 확인하여 쉽게 가입을 하며 추후 SNS가입을 연동할 예정입니다.

**구현** : <br />
createEmailPassword, Signin, authChanged등의 제공 함수를 이용하여 가입및 로그인을 구현하였습니다.
또한 프로필을 설정하여 로그인한 사용자의 닉네임을 게시글에 자동으로 작성되도록 구현되었으며
가입을하여 로그인한 사용자가 확인이 되어야만 리뷰작성이 가능하도록 하였습니다.
이는 제공해주는 {onAuthStateChanged}와 {auth}의 currentUser에서 가능합니다.

**문제사항** : <br />
제공해주는 기능에 비하여 낮은 활용성을 다루는것 같아
firebase만을 목적으로 한 연습용 프로젝트를 진행한 후 다시 이어서 리팩토링을 진행할 예정입니다.
